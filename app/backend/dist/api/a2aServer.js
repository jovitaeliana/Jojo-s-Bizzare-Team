...
import { JSONRPCErrorCode, createJSONRPCResponse, createJSONRPCError, createA2ATask, createA2AMessage, } from "../types/a2a.js";
/**
 * A2A Server Class
...
export class A2AServer {
     ...
     */
    async handleMessageSend(req, agentId) {
        try {
            const params = req.params;
            if (!params || !params.message) {
                return createJSONRPCError(JSONRPCErrorCode.INVALID_PARAMS, "Invalid params: message required", req.id);
            }
            ...
            return createJSONRPCError(JSONRPCErrorCode.INTERNAL_ERROR, error.message, req.id);
        }
    }
    /**
     * Process message and update task status
     */
    async processMessage(taskId, message, agentId) {
        ...
        try {
            ...
            if (handler) {
                ...
                const response = createA2AMessage("agent", `Received: ${message.parts[0]?.text}`, taskId);
                task.status.state = "completed";
                task.status.message = response;
                task.status.result = {
                    response: response.parts[0]?.text,
    ...
    handleTaskGet(req) {
        try {
            const params = req.params;
            if (!params || !params.id) {
                return createJSONRPCError(JSONRPCErrorCode.INVALID_PARAMS, "Invalid params: id required", req.id);
            }
            const task = this.tasks.get(params.id);
            if (!task) {
                return createJSONRPCError(JSONRPCErrorCode.TASK_NOT_FOUND, `Task not found: ${params.id}`, req.id);
            }
            return createJSONRPCResponse({ task }, req.id);
        }
        catch (error) {
 ...
 */
export function createA2ARouter(server) {
    const router = Router();
    /**
    ...
    const authenticateAPIKey = (req, res, next) => {
        ...
        if (!apiKey) {
            return res.status(401).json(createJSONRPCError(JSONRPCErrorCode.UNAUTHORIZED, "Missing X-Agent-API-Key header", null));
        }
        // In production, validate API key against database
        // For now, accept any non-empty key
        if (typeof apiKey !== "string" || apiKey.length === 0) {
            return res.status(401).json(createJSONRPCError(JSONRPCErrorCode.UNAUTHORIZED, "Invalid API key", null));
        }
        next();
    };
    /**
     ...
     */
    router.post("/api/a2a/:agentId", authenticateAPIKey, async (req, res) => {
        const { agentId } = req.params;
        const rpcRequest = req.body;
        // Validate JSON-RPC request
        if (!rpcRequest || rpcRequest.jsonrpc !== "2.0" || !rpcRequest.method) {
            return res.status(400).json(createJSONRPCError(JSONRPCErrorCode.INVALID_REQUEST, "Invalid JSON-RPC 2.0 request", rpcRequest?.id || null));
        }
        let response;
        ...
        switch (rpcRequest.method) {
            ...
            case "tasks/list":
                ...
                break;
            default:
                response = createJSONRPCError(JSONRPCErrorCode.METHOD_NOT_FOUND, `Method not found: ${rpcRequest.method}`, rpcRequest.id);
        }
        res.json(response);
    });
    return router;

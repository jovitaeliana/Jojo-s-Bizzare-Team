 ...
 */
import { createJSONRPCRequest, createA2AMessage, } from "../types/a2a.js";
/**
 * A2A Client Class
...
export class A2AClient {
     ...
     */
    async sendRequest(targetAgentId, request) {
     ...
     */
    async sendMessage(targetAgentId, text, taskId) {
     ...
     */
    async sendCustomMessage(targetAgentId, message) {
        const params = {
            message,
            stream: false,
        };
     ...
     * Get task status
     */
    async getTask(taskId, targetAgentId) {
        const params = {
            id: taskId,
        };
     ...
     * Cancel a task
     */
    async cancelTask(taskId, targetAgentId) {
        const params = {
            id: taskId,
        };
     ...
     */
    async waitForTask(taskId, targetAgentId, options) {
     ...
     */
    async sendAndWait(targetAgentId, text, options) {
     ...
     */
    async negotiate(targetAgentId, productId, offeredPrice, message) {
     ...
     */
    async confirmDelivery(targetAgentId, escrowAddress, rating) {
        const confirmationMessage = createA2AMessage("user", `Delivery confirmed for escrow ${escrowAddress}`);
     ...
     */
    async requestListings(targetAgentId, category) {
 ...
 */
export function createBuyerClient(baseUrl = "http://localhost:3000") {
 ...
 */
export function createSellerClient(baseUrl = "http://localhost:3000") {
    return new A2AClient({
        baseUrl,

...
export class ERC8004Service {
    ...
    constructor() {
        ...
        this.contract = new ethers.Contract(IDENTITY_REGISTRY_ADDRESS, IDENTITY_REGISTRY_ABI, this.provider);
    }
    /**
    ...
    connectWallet(privateKey) {
        ...
        this.contract = new ethers.Contract(IDENTITY_REGISTRY_ADDRESS, IDENTITY_REGISTRY_ABI, this.wallet);
    }
    /**
     ...
     */
    async registerAgent(name, address, capabilities, tokenURI, metadata) {
        if (!this.wallet) {
     ...
     */
    async getAgentMetadata(agentId, key) {
        try {
     ...
     * Get agent token URI
     */
    async getAgentTokenURI(agentId) {
        try {
     ...
     */
    async getAgentInfo(agentId, metadataKeys) {
        try {
    ...
    async discoverAgents(capability) {
        ...
        const agentIds = [buyerAgentId, sellerAgentId].filter((id) => !!id);
        ...
        if (capability) {
            return agents.filter(agent => agent.capabilities.some(cap => cap.name.includes(capability)));
        }
        return agents;
    }
    /**
     ...
     */
    async queryAgentCapabilities(agentId) {
        try {
     ...
     */
    async verifyAgentTrust(agentAddress) {
        try {
            const agent = Array.from(this.discoveredAgents.values()).find((a) => a.owner.toLowerCase() === agentAddress.toLowerCase());
     ...
     */
    async verifyAgentIdentity(agentId, expectedOwner) {
        try {

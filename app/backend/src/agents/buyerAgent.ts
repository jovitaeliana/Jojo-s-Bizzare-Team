...
import { hcsService, HCSEventType } from "../services/hcs.js";
...
const BuyerState = Annotation.Root({
  ...
  excludedProductIds: Annotation<string[]>({
...
export class BuyerAgent {
  ...
  private x402Initialized: boolean = false;

  // Pre-created tools for LangSmith tracing
  private x402Tool: any;
  private discoveryTool: any;
  private a2aTool: any;
   ...
   */
  async initializePayments(accountId: string, privateKey: string): Promise<void> {
  ...
  private createX402PaymentTool() {
    ...
    return tool(
      async ({ sellerAddress, amount, currency, productId }) => {
        ...
        if (!self.x402Initialized) {
          ...
          if (buyerId && buyerKey) {
            ...
            throw new Error(
              "x402 service not initialized and BUYER_ACCOUNT_ID/BUYER_PRIVATE_KEY not set"
            );
          }
        }
        ...
        if (typeof sellerAddress !== 'string' || !sellerAddress.includes('.')) {
          console.warn(
            `   ⚠️ sellerAddress '${sellerAddress}' does not look like a Hedera account ID (e.g., 0.0.1234)`
          );
        }
    ...
    return tool(
      async ({ capability }) => {
        ...
        return {
          ...
          agents: agents.map(a => ({
            agentId: a.agentId,
            name: (a as any).agentName || a.agentCard?.name || a.metadata?.find(m => m.key === 'agentName')?.value || 'Unknown',
    ...
    return tool(
      async ({ agentId, message }) => {
        ...
        if (result.status.state === "completed" && result.status.message) {
          const response = result.status.message as A2AMessage & { metadata?: any };
  ...
  private async discoverNode(state: BuyerStateType): Promise<Partial<BuyerStateType>> {
    ...
    try {
      ...
      for (const agent of discoveryResult.agents) {
        try {
          ...
          if (a2aResult.success) {
            ...
            products.push(...listingsData.map((l: any) => ({
              ...l,
      ...
      if (hcsService.isInitialized()) {
        await hcsService.logDiscovery(
          this.agentId,
          'marketplace_seller',
          discoveryResult.agents.map((a: any) => ({
            agentId: a.agentId,
            name: a.name,
            endpoint: a.endpoint,
          }))
  ...
  private async selectNode(state: BuyerStateType): Promise<Partial<BuyerStateType>> {
    ...
    try {
      console.log(`\n   🔍 DEBUG: User Request = "${state.userRequest}"`);
      ...
      const decision = this.safeParseJSON<any>(response.content) || {
  ...
  private async negotiateNode(state: BuyerStateType): Promise<Partial<BuyerStateType>> {
    ...
    try {
      ...
      const task = await this.a2aClient.sendCustomMessage(
      ...
      const result = await this.a2aClient.waitForTask(
        task.id,
        state.selectedProduct.sellerAgentId || "28",
        { pollInterval: 1000, maxAttempts: 30 }
      );

      if (result.status.state === "completed" && result.status.message) {
        const sellerResponse = result.status.message as A2AMessage & { metadata?: any };
        ...
        if (meta.accepted === true || meta.action === 'accept') {
          ...
          if (hcsService.isInitialized()) {
            await hcsService.logNegotiation(
              this.agentId,
              1,
              { price: decision.offerPrice, message: decision.message },
              undefined,
              true
            );
          }
        ...
        if (meta.action === 'counter' && typeof meta.counterOffer === 'number') {
          const counter = meta.counterOffer as number;
          ...
          if (counterMatch) {
            ...
            if (!isNaN(counter)) {
              if (!state.userBudget || counter <= state.userBudget) {
                ...
                const updatedProduct = { ...state.selectedProduct, price: counter } as Product;
                return {
                  selectedProduct: updatedProduct,
                  offerAccepted: true,
                  negotiationHistory: [...(state.negotiationHistory || []), { offer: decision, response: sellerResponse }],
  ...
  private async completeNode(state: BuyerStateType): Promise<Partial<BuyerStateType>> {
    ...
    try {
      ...
      if (hcsService.isInitialized()) {
        await hcsService.logTransactionComplete(
          this.agentId,
          state.paymentId,
          {
            product: {
   ...
   */
  private shouldNegotiate(state: BuyerStateType): "negotiate" | typeof END {
    if (state.selectedProduct) {
      return "negotiate";
    }
    return END;
   ...
   */
  private shouldPay(state: BuyerStateType): "pay" | "select" | typeof END {

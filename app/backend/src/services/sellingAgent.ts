...
export class SellingAgentService {
   ...
   */
  async publishListing(listingId: string, sellerAddress: string): Promise<boolean> {
    try {
   ...
   */
  private async registerAsSeller(address: string): Promise<void> {
    try {
      // Register with ERC-8004 agent registry
      await erc8004Service.registerAgent(
        'User Seller Agent',
        address,
        [
          {
  ...
  async updateListing(
    listingId: string,
    updates: Partial<Omit<Listing, 'id' | 'createdAt' | 'updatedAt'>>
  ): Promise<Listing> {
    try {
      const listing = this.listings.get(listingId);
      if (!listing) {
   ...
   */
  async markAsSold(listingId: string, buyerAddress: string): Promise<boolean> {
    try {
      const listing = this.listings.get(listingId);
      if (!listing) {
        throw new Error('Listing not found');
   ...
   */
  async generateListingRecommendations(userInput: string): Promise<{
